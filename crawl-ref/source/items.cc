...
static void _add_one_item_to_chain(item_def &chain, item_def &item, 
    ...
    if (item.flags & ISFLAG_KNOW_PLUSES) {
        ...
        if (!(chain.flags & ISFLAG_KNOW_PLUSES)) {
            coord_def savedpos = chain.pos;
            _add_one_item_to_chain(chain, chain, false);
            remove_item_from_chain(chain);
            chain.pos = savedpos;
        }
    } else {
    ...
    if (delvector) item.props.erase(CHAIN_VECTOR);
}
