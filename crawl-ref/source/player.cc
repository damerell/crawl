...
{
    return you.gourmand(true, !permanently)
        ? 3 : (you.get_mutation_level(MUT_HERBIVOROUS) ? -1 : 
               you.get_mutation_level(MUT_CARNIVOROUS));
}
                                   ...
                                   bool nostepdown)
{
    ...
    int enhpower = apply_enhancement(power, total_enhancers);
    if (nostepdown) {
        return enhpower;
    } else {
        return stepdown_spellpower(100 * enhpower);
    }
}
...
int player_wizardry(spell_type spell)
{
    return you.wearing(EQ_RINGS, RING_WIZARDRY)
           ...
           + you.scan_artefacts(ARTP_WIZARDRY, true)
...
vector<mutation_ac_changes> all_mutation_ac_changes = {
    ...
    ,mutation_ac_changes(MUT_SHIMMERING_SCALES,      mutation_activity_type::FULL,    {6, 6, 6})
